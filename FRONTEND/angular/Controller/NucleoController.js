angular.module('App').controller('NucleoController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, _$location, $timeout) {
    ...
    //====================Carrega o Grid
    $scope.CarregarNucleo = function () {
        $rootScope.routeloading = true;
        $scope.Nucleos = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('NucleoListar').then(function (response) {
            if (response) {
                $scope.Nucleos = response.data;
                if ($scope.Nucleos.length === 0) {
