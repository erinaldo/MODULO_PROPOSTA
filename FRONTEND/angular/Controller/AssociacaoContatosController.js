angular.module('App').controller('AssociacaoContatosController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {

    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/AssociacaoContatos@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/AssociacaoContatos@Edit").then(function (response) {
    ...
    { 'title': 'Representante', 'visible': true, 'searchable': true, 'sortable': true }
    ];
    ...
    $scope.CarregarAssociacaoContatos = function () {
        ...
        $scope.AssociacaoContatos = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('AssociacaoContatosListar').then(function (response) {
            if (response) {
                $scope.AssociacaoContatos = response.data;
                if ($scope.AssociacaoContatos.length == 0) {
    ...
    $scope.$watch('$viewContentLoaded', function () {
        $scope.ConfiguraGrid();
        $scope.CarregarAssociacaoContatos();
    });
    //$scope.EditarContato
