angular.module('App').controller('ConsultaVeiculacaoMerchaController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {
    ...
    var _Filter = JSON.parse(localStorage.getItem('ConsultaVeiculacaoMercha_filter'));
    if (!_Filter) {
    ...
    $scope.RoteiroExibir = function (pFiltro) {
        ...
        httpService.Post('ConsultaVeiculacaoMerchaGet',pFiltro).then(function (response) {
            if (response) {
                ...
                localStorage.setItem('ConsultaVeiculacaoMercha_filter', JSON.stringify($scope.Filtro));
            }
        });
    };
