angular.module('App').controller('PortadorController', ['$scope', '$rootScope', 'httpService', '$location', '$timeout', function ($scope, $rootScope, httpService, $location, $timeout) {

    //========================Verifica Permissoes
    $scope.PermissaoNew = false;
    $scope.PermissaoEdit = false;
    httpService.Get("credential/Portador@New").then(function (response) {
        $scope.PermissaoNew = response.data;
    });
    httpService.Get("credential/Portador@Edit").then(function (response) {
    ...
    //====================Carrega o Grid
    $scope.CarregarPortador = function () {
        $rootScope.routeloading = true;
        $scope.Portadores = [];
        $scope.ShowGrid = false;
        $('#dataTable').dataTable().fnDestroy();
        httpService.Get('PortadorListar').then(function (response) {
            if (response) {
                $scope.Portadores = response.data;
                if ($scope.Portadores.length === 0) {
    ...
    $scope.$watch('$viewContentLoaded', function () {
        $scope.ConfiguraGrid();
        $scope.CarregarPortador();
    });
    $scope.EditarPortador
