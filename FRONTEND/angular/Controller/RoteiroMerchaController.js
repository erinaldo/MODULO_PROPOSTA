angular.module('App').controller('RoteiroMerchaController', ['$scope', '$rootScope', '$location', 'httpService', '$location', function ($scope, $rootScope, $location, httpService, $location, $routeParams) {
    ...
    $scope.CarregarGuiaProgramas = function (pFiltro) {
        ...
        var _Url = 'RoteiroMercha/GuiaProgramacao';
        httpService.Post(_Url, pFiltro).then(function (response) {
            ...
            if (response.data.length == 0) {
                $scope.ShowGuiaProgramas = false;
                ShowAlert("Não existe Grade Merchandising de Breaks  para esse Veiculo/Data")
            }
            else {
                $rootScope.routeName = "Roteiro Merchandising ( Veiculo:" + pFiltro.Cod_Veiculo + "  Data:" + pFiltro.Data_Exibicao + " )";
    ...
    $scope.CarregarRoteiro = function (pFiltro) {
        ...
        var _Url = 'RoteiroMercha/CarregarRoteiro';
        httpService.Post(_Url, pFiltro).then(function (response) {
            if (response.data.length == 0) {
                ...
                httpService.Post("RoteiroMercha/CarregarComerciais", pFiltro).then(function (responseComercial) {
    ...
    $scope.Ordenacao = function (Index_Origem, Index_Destino) {
        ...
        newItem.Cod_Tipo_Comercial = $scope.Comerciais[Index_Origem].Cod_Tipo_Comercial;
    ...
    $scope.CortarRoteiro = function (pComercial) {
        swal({
            ...
            httpService.Post("RoteiroMercha/BaixarVeiculacao", pComercial).then(function (response) {
    ...
    $scope.ExcluirRoteiro = function (pFiltro) {
        ...
        swal({
            ...
            httpService.Post("RoteiroMercha/Excluir", _data).then(function (response) {
                if (response) {
    ...
    $scope.SalvarRoteiro = function (pRoteiro) {
        httpService.Post("RoteiroMercha/Salvar", pRoteiro).then(function (response) {
            if (response) {
                ShowAlert(response.data[0].Mensagem)
    ...
    $scope.ImprimirRoteiro = function (pRoteiro) {
        ...
        httpService.Post("RoteiroMercha/ImprimirRoteiro/" , pRoteiro).then(function (response) {
            if (response.data) {
                url = $rootScope.baseUrl + "PDFFILES/ROTEIROMERCHA/" + $rootScope.UserData.Login.trim() + "/" + response.data;
