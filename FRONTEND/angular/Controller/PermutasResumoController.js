angular.module('App').controller('PermutasResumoController', ['$scope', '$rootScope', '$location', 'httpService', '$location', '$routeParams', function ($scope, $rootScope, $location, httpService, $location, $routeParams) {
    ...
    $scope.PermissaoEntregaCliente = false;
    $scope.PermissaoEntregaVeiculo = false;
    ...
    $scope.CarregaPermuta = function (pIdPermuta) {
        ...
        httpService.Get('Permutas/GetPermuta/' + pIdPermuta).then(function (response) {
            if (response) {
                ...
                if ($scope.permuta.ItensResumoCliente[0].Id_Item_Permuta == 0) {
                    $scope.PermissaoEntregaCliente = false;
                }
                else {
                    $scope.PermissaoEntregaCliente = true;
                }

                if ($scope.permuta.ItensResumoVeiculo[0].Competencia == "") {
                    $scope.PermissaoEntregaVeiculo = false;
                }
                else {
                    $scope.PermissaoEntregaVeiculo = true;
                }



                console.log($scope.permuta);
                console.log($scope.permuta.ItensResumoCliente);
                console.log($scope.permuta.ItensPermuta);
                ...
                for (var i = 0; i < $scope.permuta.ItensResumoCliente.length; i++) {
                    if ($scope.permuta.ItensResumoCliente[i].Id_Item_Permuta ==0) {
                        $scope.permuta.ItensResumoCliente[i].Id_Item_Permuta = "";
    ...
    $scope.CalcularItensValor_Unitario = function (pPermuta, pItem) {
        ...
        pItem.Vlr_Liq_Unit = MoneyFormat(Valor_Unitario * (pct / 100));
        pItem.Valor_Tabela = MoneyFormat(Valor_Unitario * quantidade);
        pItem.Valor_Liquido = MoneyFormat((Valor_Unitario * quantidade) * (pct / 100))
    ...
    //====================Carrega o Grid
    $scope.CarregarProdutoVeiculo = function (pNumeroNegociacao) {
        $scope.ProdutosVeiculos = [];
        httpService.Post('Permutas/ProdutosVeiculos', pNumeroNegociacao).then(function (response) {
            if (response) {
