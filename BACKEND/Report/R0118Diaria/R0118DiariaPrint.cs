...
namespace PROPOSTA
{
    public partial class R0118Diaria
    {
        ...
        public String ImprimirRpt(R0118Diaria.ReportFilterModel Rpt)
        {
        ...
        private Boolean GerarPdf(ReportFilterModel Filter, String Path, String FileName)
        {
            ...
            try
            {
                ...
                String  Data_Exibicao_Ant = dtbPdf.Rows[0]["Data_Exibicao"].ToString();
                ...
                foreach (DataRow drw in dtbPdf.Rows)
                {
                    ...
                    //-------------Quebra da Data da Exibicao
                    if (Data_Exibicao_Ant != drw["Data_Exibicao"].ToString())
                    {
                        this.ImprimeTotalAbrangencia(write, doc, strAbrangencia_Ant);
                        strAbrangencia_Ant = drw["Abrangencia"].ToString();
        ...
        private void ImprimeSubCabecalho(PdfWriter ww, Document dd, DataRow drw)
        {
            ...
            clsPdf.addCell(tbSub, new pdfLibCell() { Text = "Tipo de Receita:" + drw["Tipo_Receita"].ToString(), Height = 20, FontStyle = Font.BOLD, Align = PdfPCell.ALIGN_LEFT });
            clsPdf.addCell(tbSub, new pdfLibCell() { Text = "Competência:" + drw["Competencia_Text"].ToString(), Height = 20, FontStyle = Font.BOLD, Align = PdfPCell.ALIGN_LEFT });
            ...
            clsPdf.addCell(tbSub, new pdfLibCell() { Text = "Abrangência:" + drw["Abrangencia"].ToString(), Height = 20, FontStyle = Font.BOLD, Align = PdfPCell.ALIGN_LEFT });
            clsPdf.addCell(tbSub, new pdfLibCell() { Text = "Dt Exibição:" + drw["Data_Exibicao"].ToString().ConvertToDatetime().ToString("dd/MM/yyyy"), Height = 20, FontStyle = Font.BOLD, Align = PdfPCell.ALIGN_LEFT });
